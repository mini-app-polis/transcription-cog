"""Application entrypoint for notes-ingest-cog.

Registers a single router-style Prefect deployment (``notes-ingest-cog/
notes-ingest-cog``) and starts a runner loop that polls for scheduled or
...
Railway start command: python -m notes_ingest_cog.main
...
def notes_ingest_router(mode: NotesIngestMode) -> Any:
    ...
    if target is None:
        raise ValueError(
            f"Unknown notes-ingest-cog router mode: {mode!r}. "
...
def main() -> None:
    """Register the notes-ingest-cog router deployment and serve in-process.
    ...
    LOG.info(
        log.with_log_prefix(
            log.LOG_START,
            (
                f"Serving notes-ingest-cog router — modes={sorted(_MODE_DISPATCH)} "
