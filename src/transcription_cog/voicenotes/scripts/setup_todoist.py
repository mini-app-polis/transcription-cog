...
Run with the API token in env:
    ...
    TODOIST_API_TOKEN=<token> \
      uv run python -m transcription_cog.voicenotes.scripts.setup_todoist

Or, if you've already saved the token in Doppler:

    doppler run -- uv run python -m transcription_cog.voicenotes.scripts.setup_todoist

The script reads ``TODOIST_API_TOKEN`` directly from ``os.environ``
rather than going through ``transcription_cog.voicenotes.config.settings``: at
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not api_token:
        print(
            ...
            "transcription_cog.voicenotes.scripts.setup_todoist",
            file=sys.stderr,
        )
        return 2
