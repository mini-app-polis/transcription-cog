...
(``src/transcription_cog/voicenotes/prompts/extract.md``) and the Asana post step.
...
Field model:
  ...
  - ``description`` — one paragraph weaving WHAT and WHY together. The
    body of the Asana task starts with this.
...
class ExtractedTask(BaseModel):
    ...
    title: str = Field(
        ...,
        description=(
            "Short action-oriented title for the Asana task. The prompt "
            "nudges Claude toward concise titles (shorter reads better on "
            "the board), but no hard cap is enforced here — over-strict "
            "limits caused parse failures with no upside, since Asana "
    ...
    description: str = Field(
        ...
        description=(
            ...
            "single readable body. Goes into the Asana task body above "
            "the metadata block. No length limit — Asana handles long "
            "bodies fine."
        ),
    )
    ...
    labels: list[str] = Field(
        ...
        description=(
            "Suggested tag names — open vocabulary, 0-3 short "
            "kebab-case strings that categorize the task. Claude picks "
            "freely; the validator normalizes (lowercase, kebab-case, "
            "dedupe) and caps at 3. The cap earns its keep on Asana, "
            "where a tag is a real workspace object rather than a free "
            "string, so every distinct name Claude invents becomes "
            "something that exists and has to be looked at. Empty when "
    ...
    needs_review: bool = Field(
        ...
        description=(
            ...
            "actionable content. The Asana task is still created (so "
            "nothing is silently dropped) but tagged 'review'."
        ),
    )
